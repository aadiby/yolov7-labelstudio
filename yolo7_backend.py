...
class BloodcellModel(LabelStudioMLBase):
    ...
    def __init__(self, weights=WEIGHTS,  device=DEVICE, repo=REPO, train_output=None, **kwargs):
...
#     def reset_dir(self, dir_path):#TODO refactor
...
#                     x_center = (bbox['value']['x'] / 100 ) + (bb_width/2)
#                     y_center = (bbox['value']['y'] / 100 ) + (bb_height/2)
...
#         os.system(f"python ./yolov7/train.py --workers 8 --device cpu --batch-size {batch_size} --data ./config/data.yaml --img {IMAGE_SIZE[0]} {IMAGE_SIZE[1]} --cfg ./config/model_config.yaml \
#             --weights {self.weights} --name bloodcell_trained --hyp ./config/hyp.scratch.custom.yaml --epochs {num_epochs} --exist-ok")

#         shutil.move(f"./runs/train/bloodcell_trained/weights/best.pt", MODEL_PATH)

#         print("finished training")

#         #TODO update model weights
#         return {'model_path': "LOL"}
