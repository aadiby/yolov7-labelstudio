# MODIFIED YOLOv7 to be able to run training on GPU

Modifitions on line 686 and 758 to move files to correct memory device of needed:
```
    ADD 566: self.device = device
    MOD 686: from_which_layer.append((torch.ones(size=(len(b),)) * i).to(self.device))
    MOD 758: fg_mask_inboxes = fg_mask_inboxes.to(torch.device(self.device))
```
